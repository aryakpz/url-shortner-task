...
//     console.log(88)
//     try {
//         const userUrl = await new Promise<any[]>((res, rej) => {
//             db.all('SELECT * FROM URLTABLE WHERE username = ?', [username], (err: Error, row: any) => {
//                 if (err) {
//                     rej(err)
//                 }
//                 else {
...
export const getCurrentUserUrl = async (username: string): Promise<any[]> => {
    ...
    try {
        ...
        const userUrls = await new Promise<any[]>((resolve, reject) => {
            db.all('SELECT * FROM URLTABLE WHERE username = ?',[],usernam (err: Error, rows: any[]) => {
                if (err) {
