...
//     }
// };
import { db } from './database'; // Ensure db is properly configured for SQLite

export const getCurrentUserUrl = async (username: string): Promise<any[]> => {
    try {
        // Log to check the function execution
        console.log("Fetching URLs for username:", username);

        const userUrl = await new Promise<any[]>((resolve, reject) => {
            // SQL query to get URLs for a specific username
            db.all('SELECT * FROM URLTABLE WHERE username = ?', [username], (err: Error, rows: any) => {
                if (err) {
                    console.error("Database error:", err);
                    reject(err);  // Reject if there is an error in querying
                } else {
