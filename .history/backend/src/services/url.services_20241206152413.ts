...
import { db } from './database'; // Assuming correct database import

export const getCurrentUserUrl = async (username: string): Promise<any[]> => {
    try {
        console.log("Fetching URLs for username:", username);  // Log the username

        const userUrl = await new Promise<any[]>((resolve, reject) => {
            // Log the query execution
            console.log(`Running query: SELECT * FROM URLTABLE WHERE username = ${username}`);

            // SQL query to get URLs for the given username
            db.all('SELECT * FROM URLTABLE WHERE username = ?', [username], (err: Error, rows: any) => {
                if (err) {
                    ...
                    resolve(rows);  // Resolve the promise with fetched rows
                }
            });
        });

        return userUrl;  // Return the fetched URLs
    } catch (err) {
        console.error("Error in getCurrentUserUrl:", err);  // Log any unexpected errors
