...
return new Promise((res,rej)=>{
    const getQuery="SELECT * FROM URLTABLE WHERE username= ? "
    db.run(getQuery,[username],(err:Error,row:any)=>{
        if(err){
            rej(err)
        }
        else{
            const newData=row.map((item:any)=>{
                ...item,
                Link:`http://localhost:5001/${shorturl}`
            })
            res(row)
        }
    })
