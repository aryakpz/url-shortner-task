...
export const postUser = async (req:Request, res:Response, next:) => {
    try {
        const { name, username, password } = req.body;
        if (!name || !username || !password) {
            return res.status(400).json({ message: "All fields are required" });
        }
        const hashedPassword = await hashingPassword(password);  // Assuming hashingPassword is implemented
        await addUsersToDb(name, username, hashedPassword);  // Assuming addUsersToDb is implemented
        res.status(201).json({
            message: "User added successfully",
            success: true,
            data: { name }
