...
export const Dashboard: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        const response = await axios.get(`/api/dashboard/${username}`);
        console.log("Fetched data:", response.data);         setData(response.data);  // Store the fetched data
    ...
    <div className="bg-black text-white min-h-screen">
      ...
      <div className="max-w-full flex justify-center">
        <table className="w-full m-10 border-2 border-black font-lucida">
          ...
          <tbody>
            {urls.length > 0 ? (
              // Directly map the data inside the urls array
              ...
              // If no urls are present, show a message
              <tr>
